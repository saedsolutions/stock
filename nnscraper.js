...
async function scrapeYahooFinanceNews(symbol, daysToScrape = 14) {
  ...
  try {
    ...
    // Add a longer wait for the news section
    ...
    const articleLinks = await page.$$eval('a[href*="/news/"]', (links, symbol) => {
      return links
        .filter(link => {
          // Only include links that contain the stock symbol and are news articles
...
async function scrapeReutersNews(symbol, daysToScrape = 14) {
  ...
  try {
    ...
    // Extract news article links using multiple possible selectors
    ...
    for (const { title, url } of articleLinks) {
      ...
      try {
        ...
        // Wait for article content with multiple possible selectors
        await page.waitForSelector('article, .article-body, [data-testid="article-body"], .paywall-article', { timeout: 20000 })
          .catch(() => console.log(`Warning: Could not find content for article: ${title}`));
        
        // Get publish date with multiple possible selectors
        ...
        if (publishedAt) {
          ...
          // Get main text with multiple possible selectors
...
Fixed script now filters articles by checking if they mention the stock symbol in either the title or URL.